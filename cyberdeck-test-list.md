...
The `cyberdeck_evt3` board has standalone DTS and pinctrl definitions based on `board_cyberdeck_evt3.{c,h}`. Asterix-only sources are excluded and the shell runs over USB CDC. The firmware has been flashed to the Q20 + 83_OFN EVT3 assembly and physically exercised for I²C probes, flash, sensors, display, selected keyboard/OFN input, haptic, lighting, BLE, LFXO, IMU INT1, speaker, power cycling, and DA7212 microphone capture.
...
| LSM6DSO interrupt | INT1 P0.09; P0.10 is INT2 | `src/fw/board/boards/board_cyberdeck_evt3.c` and EVT3 schematic |
...
| `hwv imu get` (`app/src/imu.c`) | Use shared I2C address 0x6b, Cyberdeck orientation, and correct power setup. Keep polling separate from the interrupt test below. | `src/fw/drivers/imu/lsm6dso/lsm6dso.c` for WHO_AM_I 0x6c, reset, range/ODR setup, axis mapping, FIFO, and interrupt behavior. | Accelerometer axes respond correctly to board rotation and gyro values respond to motion. |
| `hwv speaker play` (`app/src/speaker_cyberdeck.c`) | Use DA7212 at 0x1a and direct nrfx slave-mode streaming with fixed 4 MHz MCK, 16-bit stereo TX double buffers, and the confirmed mapping: BCLK P1.09, WCLK P0.08, MCLK P0.06, nRF SDOUT P1.08, and nRF SDIN P0.07. Require PLL status 0x07. | `src/fw/drivers/speaker/nrf5/da7212.c` for nrfx streaming, PLL/DAI setup, line/headphone routing, and checked shutdown; `src/fw/board/boards/board_cyberdeck_evt3.c` for pins. | Known sample plays clearly through the speaker without clipping or I2S errors. |
...
| IMU INT1 | Configure data-ready on firmware-assigned P0.09, fetch the accelerometer sample in the callback to clear DRDY, and require an event. | Interrupt behavior in `src/fw/drivers/imu/lsm6dso/lsm6dso.c`; INT1 pin assignment in `src/fw/board/boards/board_cyberdeck_evt3.c`. | P1 |
...
| DA7212/I2S microphone | Capture MIC1_P with direct nrfx in slave mode, fixed 4 MHz MCK, codec-master exact 16 kHz BCLK/LRCK, simultaneous RX and TX-silence buffers, and 30 dB gain; require PLL status 0x07 and apply the manufacturing-test DC-blocked RMS/DC/clipping criteria. | The `cyberdeck-firmware` MIC1 route is physically functional, while the assembled EVT3 produced no signal through MIC2 despite retained register readback and either MICBIAS source. This hardware evidence overrides the conflicting `board_cyberdeck_evt3.c` MIC2 comment/schematic symbol labeling. The reference's nRF-master 4 MHz/256× frame clock still required correction to codec-master exact 16 kHz. | P0 |
...
- Physical coverage is partial: the flashed EVT3 passed the tests listed in the implementation status, including repeated MIC1 captures with exact 16 kHz codec-master clocks. Full keyboard coverage, headphone channels, charging, rail measurements, source switchover, and battery ADC remain unverified or fixture-dependent.
